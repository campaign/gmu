...
// ---- functions -----
function rScandir($path, $exts, &$res=array()) {
    $path = rtrim($path, '/\\');
    $files = scandir($path);
    foreach ($files as $file) {
        $filepath = $path . DIRECTORY_SEPARATOR . $file;
        if (is_dir($filepath) && $file[0] != '.') {
            rScandir($filepath, $exts, $res);
        } else {
...
function parse_js_object($content){
    $content = preg_replace('/{/', 'array (', $content);
    $content = preg_replace('/}/', ');', $content);
    $content = preg_replace("/(['\"]?)([a-z0-9_]+)\\1\\s*:\\s*/ims", '"\2" =>', $content);
    $fn = create_function("", "return ".$content);
    return call_user_func($fn);
}

function component_cmp($a, $b) {
    $a = intval($a['weight']);
    $b = intval($b['weight']);
    if ($a == $b) {
        return 0;
...
//collect demos
$components = array();
$files = rScandir('./', '/demo\.info$/i');
foreach($files as $file){
    $dir = dirname($file);
    $info = parse_js_object(file_get_contents($file));
    if(empty($info['group'])) $info['group'] = 'Components';
    if(empty($components[$info['group']])){
        $components[$info['group']] = array();
    }
    $components[$info['group']][] = array_merge($info, array(
        'dir' => $dir
    ));
}

foreach($components as $key=>$val){
    usort($components[$key], 'component_cmp');
}
?><!doctype html>
<html>
<head>
    ...
    <div class="thelist">
        ...
        <div class="group">
            ...
            <?php foreach($demos as $demo):?>
                <li>
                    <a href="<?php echo $demo['dir'].'/'.$demo['main']?>">
                        <img src="<?php echo $demo['dir'].'/'.$demo['icon']?>" alt="<?php echo $demo['name']?>" />
