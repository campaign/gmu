...
test("只为加载css用",function(){
    expect(1);
    stop();
    ua.loadcss(["reset.css", "icons.default.css", "webapp/button/button.css","webapp/button/button.default.css", "webapp/dropmenu/dropmenu.css", "webapp/dropmenu/dropmenu.default.css"], function(){
        ok(true, '样式加载进来了！');
        start();
    });
});
...
test("参数 － btn", function(){
    expect(3);
    stop();
    ua.importsrc('webapp/button', function(){
