...
(function ($, undefined) {
    $.ui.create("PageSection", {
        _data:{
            _currentIdx:1,
            sliders:[$("<div></div>"), $("<div></div>"), $("<div></div>")],
        ...
        _create:function () {
            ...
            skeleton.css("position", 'relative').css("width", '300%').css('height', '100%');
            var sliders = me.data("sliders");
            for (var i in me.data("sliders")) {
                sliders[i].on("touchmove", function(e){
                    e.preventDefault();
                })
                sliders[i].css("width", "33.333%").css("height", "100%");
                skeleton.append(sliders[i]);
            }
            $(me.data("sliders")[0]).append(content[0].widget());
...
//                }, 400);
        ...
        _setCSSX:function (obj, value) {
            obj.css("-webkit-transform", "translate3d(" + value + "px , 0px, 0px)");
        },
