	...
	// 
//	      http://tangram.baidu.com/license.js
	//
	...
	*/
	 ...
	 * @remark
	 * 使用者应保证提供的className合法性，不应包含不合法字符，className合法字符参考：http://www.w3.org/TR/CSS2/syndata.js。
	 * @shortcut addClass
	 * @meta standard
...
//	            if (!b.opera || parent.tagName != 'TR') {
	 ...
	 * @remark 使用者应保证提供的className合法性，不应包含不合法字符，className合法字符参考：http://www.w3.org/TR/CSS2/syndata.js。
	 ...
	 * @function
	 * @grammar baidu.dom.insertHTML(element, position, js)
	 * @param {HTMLElement|string} element 目标元素或目标元素的id
	...
	baidu.dom.query = (function (){
	    ...
	    function process(seq){
	        ...
	        while (k --) {
	            ...
	            // 识别:root,js|head|body|title等全局仅一个的标签的选择器，忽略*选择器
	            // 合并类选择器以便于使用getElementsByClassName
	            if (simple.kind == ':js') simple = make('T', 'js');
	            if (simple.kind == '=') {
...
//	            }
	    ...
	    Q._isXHTML = function (doc){
	        return doc.documentElement.nodeName == 'js';
	    };
	 ...
	 * @param {String} [resources.type]        取值["css","js","js"]，默认参考文件后缀.
	 ...
	 *  baidu.page.load([
	 *      {
	 *          url : "http://xxx.baidu.com/xpath/template.js",
	...
	baidu.page.load = function(resources, options, ignoreAllLoaded) {
	    ...
	    function loadResources(res) {
	        ...
	        res.requestType = res.requestType || (res.type == 'js' ? 'ajax' : 'dom');
